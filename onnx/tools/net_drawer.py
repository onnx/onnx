...
def GetOpNodeProducer(  # noqa: N802
    ...
    def really_get_op_node(op: NodeProto, op_id: int) -> pydot.Node:
        ...
        if embed_docstring and op.doc_string:
            node.set_tooltip(html.escape(op.doc_string))
        return node
...
def GetPydotGraph(  # noqa: N802
    ...
    if node_producer is not None and embed_docstring:
        warnings.warn(
            "embed_docstring has no effect when a custom node_producer is provided",
            stacklevel=2,
        )
    ...
    for op_id, op in enumerate(graph.node):
        ...
        for input_name in op.input:
            if not input_name:  # optional inputs are represented as empty strings
                continue
        ...
        for output_name in op.output:
            if not output_name:  # optional outputs are represented as empty strings
                continue
...
def main() -> None:
    ...
    parser.add_argument(
        "--embed_docstring",
        action="store_true",
        help="Embed docstring as an SVG tooltip on each op node.",
    )
...
get_op_node_producer = GetOpNodeProducer
get_pydot_graph = GetPydotGraph
