...
def make_tensor_sequence_map_value_info(
    name: str,
    key_type: int,
    elem_type: int,
    shape: Optional[Sequence[Union[str, int, None]]],
    ...
    """Makes a Map[Key, Sequence[Tensors]] ValueInfoProto based on the data type and shape."""
    value_info_proto = ValueInfoProto()
    value_info_proto.name = name
    if doc_string:
        value_info_proto.doc_string = doc_string
    ...
    tensor_type_proto = make_tensor_type_proto(elem_type, shape, elem_shape_denotation)
    sequence_type_proto = make_sequence_type_proto(tensor_type_proto)
    map_type_proto = make_map_type_proto(key_type, sequence_type_proto)
    value_info_proto.type.map_type.CopyFrom(map_type_proto.map_type)
