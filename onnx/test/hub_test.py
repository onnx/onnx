...
class TestModelHub(unittest.TestCase):
    ...
    def test_force_reload(self):  # type: () -> None
        model = hub.load(self.name_1, self.repo_1, force_reload=True)
    ...
    def test_listing_models(self):  # type: () -> None
        model_info_list_1 = hub.list_models(self.repo_1, tags=["vision"])
        model_info_list_2 = hub.list_models(self.repo_1)
    ...
    def test_basic_usage(self):  # type: () -> None
        model = hub.load(self.name_1, self.repo_1)
        self.assertIsInstance(model, ModelProto)
    ...
    def test_download_with_opset(self):  # type: () -> None
        model = hub.load(self.name_1, self.repo_1, opset=12)
        self.assertIsInstance(model, ModelProto)

    def test_opset_error(self):  # type: () -> None
        self.assertRaises(AssertionError, lambda: hub.load(self.name_1, self.repo_1, opset=-1))

    def test_manifest_not_found(self):  # type: () -> None
        self.assertRaises(AssertionError, lambda: hub.load(self.name_1, "mhamilton723/models:unknown"))
