...
class TestAutomaticUpgrade(unittest.TestCase):
    ...
    def _test_op_upgrade(
        ...
        is_sequence = [
            0 if id not in seq_inputs else 1 for id in range(n_inputs)]
        is_optional = [
            0 if id not in optional_inputs else 1 for id in range(n_inputs)]
        # turn empty strings into [0] to ease type analysis, even though those entries
        # will be ignored
        input_shapes_cast = cast(List[List[int]],
                                 [[0] if isinstance(
                                     shape, str) else shape for shape in input_shapes]
                                 )
    ...
    def test_Add_3(self) -> None:
        self._test_op_upgrade('Add', 1, [[3, 4, 5], [3]],
                              attrs={'consumed_inputs': [
                                  0], 'broadcast': 1, 'axis': 0}
                              )
    ...
    def test_Cast(self) -> None:
        ...
        self._test_op_upgrade('Cast', 6, [[2, 3]], [[2, 3]], [
                              TensorProto.INT64], attrs={'to': 1})
    ...
    def test_Clip_2(self) -> None:
        self._test_op_upgrade(
            'Clip', 1, attrs={'consumed_inputs': [0], 'min': -1.4})

    def test_Clip_3(self) -> None:
        self._test_op_upgrade(
            'Clip', 1, attrs={'consumed_inputs': [0], 'max': 2.6})
    ...
    def test_ConvTranspose(self) -> None:
        self._test_op_upgrade('ConvTranspose', 1, [
                              [1, 1, 5, 5], [1, 1, 3, 3]], [[1, 1, 7, 7]])
    ...
    def test_Div(self) -> None:
        self._test_op_upgrade('Div', 1, [[3, 4, 5], [3, 1, 5]], attrs={
                              'consumed_inputs': [0]})

    def test_Dropout(self) -> None:
        self._test_op_upgrade('Dropout', 1, attrs={
                              'consumed_inputs': [0], 'is_test': 1})
    ...
    def test_Equal(self) -> None:
        # 6->7 adapter is missing
        self._test_op_upgrade('Equal', 7, [[2, 3], [2, 3]], [
                              [2, 3]], output_types=[TensorProto.BOOL])
    ...
    def test_Flatten(self) -> None:
        self._test_op_upgrade('Flatten', 1, [[3, 4, 5]], [
                              [3, 20]], attrs={'axis': 1})
    ...
    def test_GlobalAveragePool(self) -> None:
        self._test_op_upgrade('GlobalAveragePool', 1, [
                              [1, 3, 10, 10]], [[1, 3, 1, 1]])

    def test_GlobalMaxPool(self) -> None:
        self._test_op_upgrade('GlobalMaxPool', 1, [
                              [1, 3, 10, 10]], [[1, 3, 1, 1]])

    def test_GlobalLpPool(self) -> None:
        # 1->2 adapter is missing
        self._test_op_upgrade('GlobalLpPool', 2, [
                              [1, 3, 10, 10]], [[1, 3, 1, 1]])
    ...
    def test_If(self) -> None:
        sub_output = [helper.make_tensor_value_info(
            'out', TensorProto.FLOAT, [3, 4, 5])]
        ...
        then_graph = helper.make_graph(
            [then_node], 'then_graph', [], sub_output, [])
        ...
        else_graph = helper.make_graph(
            [else_node], 'else_graph', [], sub_output, [])
        self._test_op_upgrade('If', 1, [[0]], [[3, 4, 5]], [TensorProto.BOOL],
                              attrs={'then_branch': then_graph,
    ...
    def test_IsInf(self) -> None:
        self._test_op_upgrade('IsInf', 10, [[2, 3]], [
                              [2, 3]], output_types=[TensorProto.BOOL])

    def test_IsNaN(self) -> None:
        self._test_op_upgrade('IsNaN', 9, [[2, 3]], [
                              [2, 3]], output_types=[TensorProto.BOOL])
    ...
    def test_Less(self) -> None:
        # 6->7 adapter is missing
        self._test_op_upgrade('Less', 7, [[2, 3], [2, 3]], [
                              [2, 3]], output_types=[TensorProto.BOOL])
    ...
    def test_LSTM_3(self) -> None:
        # 6->7 adapter is missing
        self._test_op_upgrade('LSTM', 7,
                              [[5, 3, 4], [1, 24, 4], [1, 24, 4], [1, 48],
                                  [5], [1, 5, 6], [1, 5, 6], [1, 18]],
                              [[5, 1, 3, 6], [1, 3, 6], [1, 3, 6]],
                              [TensorProto.FLOAT, TensorProto.FLOAT, TensorProto.FLOAT, TensorProto.FLOAT,
                               TensorProto.INT64, TensorProto.FLOAT, TensorProto.FLOAT, TensorProto.FLOAT],
                              attrs={'hidden_size': 6}
                              )
    ...
    def test_MeanVarianceNormalization(self) -> None:
        self._test_op_upgrade('MeanVarianceNormalization',
                              9, attrs={'axes': [1, 2]})
    ...
    def test_Mod_2(self) -> None:
        self._test_op_upgrade('Mod', 10, [[2, 3], [2, 3]], [
                              [2, 3]], attrs={'fmod': 1})
    ...
    def test_NonZero(self) -> None:
        self._test_op_upgrade('NonZero', 9, [[3, 3]], [
                              [2, 4]], output_types=[TensorProto.INT64])

    def test_Not(self) -> None:
        self._test_op_upgrade('Not', 1, [[2, 3]], [[2, 3]], [
                              TensorProto.BOOL], [TensorProto.BOOL])
    ...
    def test_QLinearMatMul(self) -> None:
        self._test_op_upgrade('QLinearMatMul', 10, [[2, 3], [], [], [
                              3, 4], [], [], [], []], [[2, 4]])
    ...
    def test_RandomNormal(self) -> None:
        self._test_op_upgrade('RandomNormal', 1, [], [
                              [3, 4, 5]], attrs={'shape': [3, 4, 5]})
    ...
    def test_RandomUniform(self) -> None:
        self._test_op_upgrade('RandomUniform', 1, [], [
                              [3, 4, 5]], attrs={'shape': [3, 4, 5]})
    ...
    def test_Range(self) -> None:
        start = helper.make_tensor(
            'a', TensorProto.FLOAT, dims=[], vals=np.array([0]))
        end = helper.make_tensor('b', TensorProto.FLOAT,
                                 dims=[], vals=np.array([12]))
        step = helper.make_tensor(
            'c', TensorProto.FLOAT, dims=[], vals=np.array([2]))
    ...
    def test_Shape(self) -> None:
        self._test_op_upgrade('Shape', 1, [[3, 4, 5]], [
                              [3]], output_types=[TensorProto.INT64])
    ...
    def test_Size(self) -> None:
        self._test_op_upgrade('Size', 1, [[3, 4, 5]], [
                              []], output_types=[TensorProto.INT64])

    def test_Slice(self) -> None:
        self._test_op_upgrade('Slice', 1, [[3, 4, 5]], [[3, 2, 2]],
                              attrs={'axes': [1, 2], 'starts': [
                                  0, 1], 'ends': [2, 3]}
                              )
    ...
    def test_TfIdfVectorizer(self) -> None:
        self._test_op_upgrade('TfIdfVectorizer', 9, [[3]], [[5]],
                              attrs={'max_gram_length': 3, 'max_skip_count': 1, 'min_gram_length': 2,
                                     'mode': 'TFIDF', 'ngram_counts': [0, 20], 'ngram_indexes': [3, 4]}
                              )
    ...
    def test_Tile(self) -> None:
        # 5->6 adapter is missing
        repeats = helper.make_tensor('b', TensorProto.INT64, dims=[
                                     3], vals=np.array([1, 2, 3]))
    ...
    def test_Unsqueeze(self) -> None:
        self._test_op_upgrade('Unsqueeze', 1, [[3, 4, 5]], [
                              [3, 4, 1, 5]], attrs={'axes': [2]})
    ...
    def test_DFT(self) -> None:
        ...
        self._test_op_upgrade('DFT', 17, [[2, 16, 1], []], [
                              [2, 9, 2]], attrs={'onesided': 1})
        self._test_op_upgrade('DFT', 17, [[2, 16, 2], []], [
                              [2, 9, 2]], attrs={'onesided': 1})
        self._test_op_upgrade('DFT', 17, [[2, 16, 1], []], [
                              [2, 16, 2]], attrs={'inverse': 1})
        self._test_op_upgrade('DFT', 17, [[2, 16, 2], []], [
                              [2, 16, 2]], attrs={'inverse': 1})

    def _test_short_time_fourier_transform(self, operator_name: str) -> None:
        # Real
        signal = helper.make_tensor('a', TensorProto.FLOAT, dims=[
                                    2, 64], vals=np.random.rand(2, 64).astype(np.float32))
        frame_step = helper.make_tensor(
            'b', TensorProto.INT64, dims=[1], vals=np.array([8]))
        window = helper.make_tensor('c', TensorProto.FLOAT, dims=[
                                    16], vals=np.ones(16).astype(np.float32))
        ...
        # Complex
        signal = helper.make_tensor('a', TensorProto.FLOAT, dims=[
                                    2, 64, 2], vals=np.random.rand(2, 64, 2).astype(np.float32))
        frame_step = helper.make_tensor(
            'b', TensorProto.INT64, dims=[1], vals=np.array([8]))
        window = helper.make_tensor('c', TensorProto.FLOAT, dims=[
                                    16], vals=np.ones(16).astype(np.float32))
    ...
    def test_ops_tested(self) -> None:
        ...
        all_op_names = [
            schema.name for schema in all_schemas if schema.domain == '']
        excluded_ops = [
