...
class TestStubAssertionLogic(unittest.TestCase):
    ...
    def test_no_cmake_args(self) -> None:
        ...
        # When CMAKE_ARGS is not set, should default to False (stubs enabled)
        # This simulates the else branch in CmakeBuild.run()
    ...
    def test_assertion_logic_stubs_disabled_no_files(self) -> None:
        ...
        # Should not raise assertion error (skipped)
        try:
            if not stubs_disabled:
                assert generated_pyi_files, "Bug: No .pyi files"
            success = True
        ...
        self.assertTrue(success, "Should not raise assertion when stubs disabled")
...
class TestErrorMessages(unittest.TestCase):
    ...
    def test_error_message_contains_fix_instructions(self) -> None:
        ...
        self.assertIn("protobuf < 21.0", error_message)
