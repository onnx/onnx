...
class TestShapeInference(unittest.TestCase):
    ...
    def test_imagetocol_padding(self):  # type: () -> None
        graph = self._make_graph(
            [('x', TensorProto.FLOAT, (3, 4, 5, 6))],
            [make_node('ImageToCol', ['x'], 'y', block_shape=[2, 2], pads=[1, 1, 1, 1], strides=[1, 1])],
            [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (3, 16, 42))])

    def test_imagetocol_no_padding(self):  # type: () -> None
        graph = self._make_graph(
            [('x', TensorProto.FLOAT, (1, 1, 5, 5))],
            [make_node('ImageToCol', ['x'], 'y', block_shape=[3, 3])],
            [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (1, 9, 9))])

    def test_imagetocol_padding_stride(self):  # type: () -> None
        graph = self._make_graph(
            [('x', TensorProto.FLOAT, (3, 4, 5, 5))],
            [make_node('ImageToCol', ['x'], 'y', block_shape=[3, 3], pads=[2, 2, 2, 2], strides=[3, 3])],
            [])
