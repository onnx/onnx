...
def convert(stem, package_name, output, do_onnx_ml=False, lite=False, protoc_path=''):  # type: (Text, Text, Text, bool, bool, Text) -> None
    ...
    with io.open(proto_in, 'r') as fin:
        ...
        if need_rename:
            fname_tmp = "{}-ml.pb.h" if do_onnx_ml else "{}.pb.h"
            proto_header = qualify(fname_tmp.format(stem), pardir=output)
    ...
    if need_rename:
        ...
        fname_tmp = '{}_ml_pb2.py' if do_onnx_ml else '{}_pb2.py'
        pb2_py = qualify(fname_tmp.format(stem.replace('-', '_')), pardir=output)
