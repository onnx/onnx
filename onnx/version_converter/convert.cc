...
void DefaultVersionConverter::convert_graph(
  ...
  while (curr_version != target_version.version()) {
    ...
    while (it != g->end()) {
      ...
      if (op_name != "Undefined" && op_name != "Captured" && all_schemas.find(op_name) != all_schemas.end()) {
...
//DefaultVersionConverter::convert_function(const FunctionProto& func_in, const OpSetID& initial_version,
//    const OpSetID& target_version) const {
...
ModelProto DefaultVersionConverter::convert_version(
  ...
  std::vector<std::shared_ptr<Graph>> function_graphs = ImportFunctionProto(mp_in);
  for (auto f_g : function_graphs) {
    convert_graph(f_g, initial_version, target_version);
  }

  // Export g as ModelProto
  debug("Finished conversion; returning model");
  ModelProto mp_out = PrepareOutput(mp_in);
  ExportModelProto(&mp_out, g, mp_in, function_graphs);
  return mp_out;
}
