...
class Reshape_5_4 final : public Adapter {
  public:
    ...
    void adapt_reshape_5_4(std::shared_ptr<Graph> graph, Node* node) const {
      // TODO: Identify if shape is statically determined; if so, feed as attribute
      const ArrayRef<Value*>& inputs = node->inputs();
      // TODO: Confirm that second input will always be shape (and first will be data)
      ...
      node->removeInput(1);
      node->is_(kshape, std::move(shape));
    }
