...
class Range(Base):
    ...
    def export_range_positive_delta():  # type: () -> None
        ...
        output = np.arange(start[0], end[0], delta[0], dtype=np.float32)  # expected output [1.0, 3.0]
        expect(node, inputs=[start, end, delta], outputs=[output],
    ...
    def export_range_negative_delta():  # type: () -> None
        ...
        end = np.array([6.0], dtype=np.float32)
        delta = np.array([-3.0], dtype=np.float32)        
        output = np.arange(start[0], end[0], delta[0], dtype=np.float32)  # expected output [10.0, 7.0]
        expect(node, inputs=[start, end, delta], outputs=[output],
