                    ...
                    for i in range(y.shape[0]):
                        for j in range(y.shape[1]):
                            for k in range(y.shape[2]):
                                ...
                                y[:,j,k] = np.max(x[:, startH: endH, startW: endW].reshape((batch*channel, -1)), axis=1)
                    x = x.reshape(batch, channel, x.shape[1], x.shape[2])
                    y = y.reshape(batch, channel, y.shape[1], y.shape[2])
                    # Check result:
                    expect(node, inputs=[x], outputs=[y],
                       name='test_max_pooling_2D_with_pad_%d_%d_%d_%d_kernel_%d_%d_stride_%d_%d' % (
                    ...
                    for j in range(y.shape[1]):
                        for k in range(y.shape[2]):
                            for m in range(y.shape[3]):
                               ...
                               y[:,j,k,m] = np.max(x[:, startH: endH, startW: endW, startD: endD].reshape((batch*channel, -1)), axis=1)
                    x = x.reshape(batch, channel, x.shape[1], x.shape[2], x.shape[3])
                    y = y.reshape(batch, channel, y.shape[1], y.shape[2], y.shape[3])
                    # Check result:
                    expect(node, inputs=[x], outputs=[y],
                       name='test_max_pooling_3D_with_pad_%d_%d_%d_%d_%d_%d_kernel_%d_%d_%d_stride:%d_%d_%d' % (
