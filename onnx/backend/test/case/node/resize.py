...
class Resize(Base):
    ...
    def export_resize_upsample_scales_nearest() -> None:
        ...
        #    [3. 3. 3. 4. 4. 4.]
        #    [3. 3. 3. 4. 4. 4.]]]]
        output = interpolate_nd(data, nearest_coeffs, scale_factors=scales).astype(
            np.float32
        )
    ...
    def export_resize_upsample_sizes_nearest() -> None:
        ...
        #    [3. 3. 3. 3. 4. 4. 4. 4.]
        #    [3. 3. 3. 3. 4. 4. 4. 4.]]]]
        output = interpolate_nd(data, nearest_coeffs, output_size=sizes).astype(
            np.float32
        )
    ...
    def export_resize_upsample_scales_linear() -> None:
        ...
        #    [2.5  2.75 3.25 3.5 ]
        #    [3.   3.25 3.75 4.  ]]]]
        output = interpolate_nd(data, linear_coeffs, scale_factors=scales).astype(
            np.float32
        )
    ...
    def export_resize_upsample_scales_cubic() -> None:
        ...
        output = interpolate_nd(data, cubic_coeffs, scale_factors=scales).astype(
            np.float32
        )
    ...
    def export_resize_upsample_sizes_cubic() -> None:
        ...
        output = interpolate_nd(data, cubic_coeffs, output_size=sizes).astype(
            np.float32
        )
    ...
    def export_resize_upsample_scales_cubic_asymmetric() -> None:
        ...
        output = interpolate_nd(
            data,
            lambda x: cubic_coeffs(x, A=-0.75),
            scale_factors=scales,
            coordinate_transformation_mode="asymmetric",
        ).astype(np.float32)
    ...
    def export_resize_upsample_sizes_nearest_floor_align_corners() -> None:
        ...
        output = interpolate_nd(
            data,
            lambda x: nearest_coeffs(x, mode="floor"),
            output_size=sizes,
            coordinate_transformation_mode="align_corners",
        ).astype(np.float32)
    ...
    def export_resize_upsample_sizes_nearest_round_prefer_ceil_asymmetric() -> None:
        ...
        output = interpolate_nd(
            data,
            lambda x: nearest_coeffs(x, mode="round_prefer_ceil"),
            output_size=sizes,
            coordinate_transformation_mode="asymmetric",
        ).astype(np.float32)
    ...
    def export_resize_upsample_sizes_nearest_ceil_half_pixel() -> None:
        ...
        #    [13. 14. 14. 15. 15. 16. 16. 16.]]]]
        output = interpolate_nd(
            data, lambda x: nearest_coeffs(x, mode="ceil"), output_size=sizes
        ).astype(np.float32)
