...
class Equal(Base):
    ...
    def export() -> None:
        ...
        x = (np.random.randn(3, 4, 5) * 10).astype(np.uint8)
        y = (np.random.randn(3, 4, 5) * 10).astype(np.uint8)
        z = np.equal(x, y)
        expect(node, inputs=[x, y], outputs=[z], name="test_equal_uint8")

        x = (np.random.randn(3, 4, 5) * 10).astype(np.uint16)
        y = (np.random.randn(3, 4, 5) * 10).astype(np.uint16)
        z = np.equal(x, y)
        expect(node, inputs=[x, y], outputs=[z], name="test_equal_uint16")

        x = (np.random.randn(3, 4, 5) * 10).astype(np.uint32)
        y = (np.random.randn(3, 4, 5) * 10).astype(np.uint32)
        z = np.equal(x, y)
        expect(node, inputs=[x, y], outputs=[z], name="test_equal_uint32")

        x = (np.random.randn(3, 4, 5) * 10).astype(np.uint64)
        y = (np.random.randn(3, 4, 5) * 10).astype(np.uint64)
        z = np.equal(x, y)
