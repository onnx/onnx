...
PYBIND11_MODULE(onnx_cpp2py_export, onnx_cpp2py_export) {
  ...
  shape_inference.def(
      "infer_shapes",
      [](const py::bytes& bytes, bool check_type, bool strict_mode) {
        ModelProto proto{};
        ParseProtoFromPyBytes(&proto, bytes);
        shape_inference::InferShapes(proto, check_type, OpSchemaRegistry::Instance(), strict_mode == true ? 1 : 0);
        std::string out;
        proto.SerializeToString(&out);
        return py::bytes(out);
      },
  ...
  shape_inference.def(
      ...
      [](const std::string& model_path, const std::string& output_path, bool check_type, bool strict_mode) -> void {
        shape_inference::InferShapes(
            model_path, check_type, output_path, OpSchemaRegistry::Instance(), strict_mode == true ? 1 : 0);
      });
