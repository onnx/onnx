...
def construct_original_grid(data_size, align_corners, mod):
    ...
    for dim, dim_size in enumerate(data_size):
        ...
        if dim == 0:
            if is_2d:
                y = mod.reshape(a, (dim_size, 1)) + size_zeros
                original_grid = [y, *original_grid]
            else:
                z = mod.reshape(a, (dim_size, 1, 1)) + size_zeros
                original_grid = [z, *original_grid]
        elif dim == 1:
            if is_2d:
                x = mod.reshape(a, (1, dim_size)) + size_zeros
                original_grid = [x, *original_grid]
            else:
                y = mod.reshape(a, (1, dim_size, 1)) + size_zeros
                original_grid = [y, *original_grid]
        else:
            x = mod.reshape(a, (1, dim_size)) + size_zeros
            original_grid = [x, *original_grid]
    return mod.stack(original_grid, axis=2 if is_2d else 3)
...
def apply_affine_transform(theta_n, original_grid_homo):
    ...
    if original_grid_homo.ndim == 3:
        ...
        original_grid_transposed = mod.transpose(
            mod.reshape(original_grid_homo, (H * W, dim_homo))
        )
        grid_n = mod.matmul(
            theta_n, original_grid_transposed
        )  # shape (N, dim_2d, H * W)
        # transpose to (N, H * W, dim_2d) and then reshape to (N, H, W, dim_2d)
        grid = mod.reshape(mod.transpose(grid_n, (0, 2, 1)), (N, H, W, dim_2d))
        return grid.astype(mod.float32)
    else:
        ...
        original_grid_transposed = mod.transpose(
            mod.reshape(original_grid_homo, (D * H * W, dim_homo))
        )
        grid_n = mod.matmul(
            theta_n, original_grid_transposed
        )  # shape (N, dim_3d, D * H * W)
        # transpose to (N, D * H * W, dim_3d) and then reshape to (N, D, H, W, dim_3d)
        grid = mod.reshape(mod.transpose(grid_n, (0, 2, 1)), (N, D, H, W, dim_3d))
        return grid.astype(mod.float32)
