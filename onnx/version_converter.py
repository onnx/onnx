...
def convert_version(model, target_version):  # type: (ModelProto, int) -> ModelProto
    if not isinstance(model, ModelProto):
        raise TypeError('VersionConverter only accepts ModelProto as model, incorrect type: {}'.format(type(model)))
    if not isinstance(target_version, int):
        raise TypeError('VersionConverter only accepts int as target_version, incorrect type: {}'.format(type(target_version)))
