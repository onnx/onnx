...
#define DEFINE_PARSE_DATA(type, typed_data_fetch, tensorproto_datatypes)           \
  template <>                                                                      \
  const std::vector<type> ParseData(const TensorProto* tensor_proto) {             \
    ...
    } else if (tensorproto_datatypes.count(tensor_proto->data_type()) == 0) {      \
      std::string supported_types = "{";                                           \
      for (auto datatype: tensorproto_datatypes) {                                 \
...
const std::unordered_set<int> int32_support_types {
...
const std::unordered_set<int> float_support_types {
