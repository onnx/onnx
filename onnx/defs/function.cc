...
namespace ONNX_NAMESPACE {
  ...
  Status FunctionBuilderRegistry::GetFunctions(
    ...
    for (auto func_builder : function_builders) {
      ...
      auto version_range = OpSchemaRegistry::DomainToVersionRange::Instance().Map().at(func_builder.GetDomain());
      op_set.insert({ 
        func_builder.GetDomain(), 
        function_proto->since_version() < version_range.second ? function_proto->since_version() : version_range.second
      });
