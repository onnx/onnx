...
static Common::Status BuildMVN(std::unique_ptr<FunctionProto>* func_proto) {
  ...
  //    normalization " "on the input tensor X using formula: <br/> ```
  //    (X-EX)/sqrt(E(X-EX)^2) ``` <br/><br/>"
  //    "<b>INPUT: </b>X(float/float16/double) with shape [N,C,W,H] or N-D shape
  //    <br/><br/>"
  //    "<b>ATTRIBUTE: </b><br/>&nbsp;&nbsp;&nbsp;&nbsp;<tt>axes: </tt>will be
  //    passed to ReducedMean " "Ops. Use [0,2,3] (without C axis for N-D cases)
  //    for calculating means and variances " "along channels. Two variables
  //    with the same C-coordinate are associated " "with the same mean and
  //    variance. Use [0,1,2,3] (with C axis) to calculate " "global mean and
  //    global variance with all variables sharing the same mean/variance.<br/>"
  //    "&nbsp;&nbsp;&nbsp;&nbsp;(The KeepDims attribute in ReducedMean is set
  //    to true for calculation)<br/>"
  //    "<br/><b>OUTPUT: </b>X_MVN(float/float16/double) with the same shape as
  //    input X<br/>");
  ...
  //    "Exponent (default to 2.0) to element-wisely calculate the square of a
  //    tensor");
  ...
  //    "Epsilon (default to 1e-9) to element-wisely add to the divisor
  //    tensor");
