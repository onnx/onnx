...
is applied to the tensor elementwise.
    ...
    .Attr("alpha", "Threshold value", AttributeProto::FLOAT, OPTIONAL)
...
OPERATOR_SCHEMA(Embedding)
    ...
    .AllowConsumed({{0, 0}})
    .SetDoc(
        R"DOC(Turns positive integers (indexes) into dense vectors of fixed size.)DOC")
    .Attr(
        "input_dim",
    ...
    .Input(
        ...
        "with length [N] and values [0, input_dim -1]",
        {DataType::Tensor_INT64})
    .Output(0, "output", "Output tensor of computed features [N, O].", "T")
    .TypeConstraint(
        "T",
        {DataType::Tensor_FLOAT16,
         DataType::Tensor_FLOAT,
