...
void IfInferenceFunction_16(InferenceContext& ctx) {
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .TypeAndShapeInferenceFunction(IfInferenceFunction_16));

void ClearShape_opset16(TypeProto& input_type) {
  if (input_type.has_tensor_type()) {
    ...
    if (seq_type.has_elem_type()) {
      ClearShape_opset16(*(seq_type.mutable_elem_type()));
    }
  } else if (input_type.has_optional_type()) {
    auto& opt_type = *input_type.mutable_optional_type();
    if (opt_type.has_elem_type()) {
      ClearShape_opset16(*(opt_type.mutable_elem_type()));
    }
  }
}

void LoopInferenceFunction_16(InferenceContext& ctx) {
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .TypeAndShapeInferenceFunction(LoopInferenceFunction_16));

int handle_negative_axis_validate_opset16(const std::string& attrib, int axis, int rank) {
...
void ScanInferenceFunction_16(InferenceContext& ctx) {
  auto num_inputs = ctx.getNumInputs();
  ...
  for (size_t i = 0; i < num_inputs; ++i) {
    ...
    if (is_loop_state_var) {
      ...
      if (has_shape) {
        ...
        axis = handle_negative_axis_validate_opset16("scan_input_axes", axis, shape.dim_size());
    ...
    for (size_t i = 0; i < num_outputs; ++i) {
      ...
      if (is_loop_state_var) {
        ...
        if (subgraph_output_tensor_type.has_shape()) {
          ...
          output_axis = handle_negative_axis_validate_opset16("scan_output_axes", output_axis, output_rank);
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .TypeAndShapeInferenceFunction(ScanInferenceFunction_16)); // Shares same shape inference as opset 11
