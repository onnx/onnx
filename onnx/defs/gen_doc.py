...
def display_function(function, versions, domain=ONNX_DOMAIN):  # type: (FunctionProto, List[int], Text) -> Text
    ...
    if len(versions) > 1:
        s += '\nOther versions of this function: {}\n'.format(
            ', '.join(display_function_version_link(domain_prefix + 'Func' + function.name, v) for v in versions if v != function.since_version))
...
def main(args):  # type: (Type[Args]) -> None
    with io.open(args.changelog, 'w', newline='') as fout:
        fout.write('## Operator/Function Changelog\n')
        fout.write(
        ...
        for domain, versionmap in sorted(dv_index.items()):
            ...
            fout.write(s)
            all_functions = defs.get_functions(domain)
            if all_functions:
                fout.write('### {} \n'.format('Functions:'))
                domain_prefix = '' if domain == '' else display_domain_short(domain)
                ...
                for version, function_list in sorted(changelog_versionmap.items()):
                    ...
                    for function in function_list:
                        s += '## Version {} of domain {}\n'.format(version, display_domain_short(domain))
                        name_with_ver = '{}-{}'.format(domain_prefix +
                                                       'Func' + fn_name, function.since_version)
                        s += '### <a name="{}"></a>**{}**</a>\n'.format(name_with_ver, name_with_ver)
    ...
    with io.open(args.output, 'w', newline='') as fout:
                ...
                for function_name, functions in sorted(all_functions.items()):
                    for function in sorted(functions, key=lambda s: s.since_version, reverse=True):
                        ...
                        s = '  * {}<a href="#{}">{}</a>\n'.format(
                            ...
                            'Func' + domain_prefix + function.name, domain_prefix + function.name)
                        fout.write(s)
                    fout.write('\n')
        ...
        if all_functions:
            fout.write('### {} \n'.format('Functions:'))
            fout.write('*Functions in ONNX is defined as optional composed operators. '
                       'Frameworks can support it directly or via decomposing it into simple '
                       'operators.*\n')
            for function_name, functions in sorted(all_functions.items()):
                ...
                s = '### {}<a name="{}"></a><a name="{}">**{}**</a>\n'.format(
                    function_status_str(function.status),
                    'Func' + domain_prefix + function.name, 'func' + domain_prefix + function.name.lower(),
