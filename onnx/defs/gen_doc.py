...
def main(args: Args) -> None:
        ...
        for domain, versionmap in sorted(dv_index.items()):
            ...
            for version, unsorted_schemas in sorted(versionmap.items()):
                ...
                for schema in sorted(unsorted_schemas, key=lambda s: s.name):
                    ...
                    s += (
                        f'### <a name="{name_with_ver}"></a>**{name_with_ver}**'
    ...
    with open(
        ...
        fout.write(
            f"*This file is automatically generated from the\n"
            f"            [def files](/onnx/defs) via [this script](/onnx/defs/gen_doc.py).\n"
            f"            Do not modify directly and instead edit operator definitions.*\n"
            f"\n"
            f"For an operator input/output's differentiability, it can be differentiable,\n"
            f"            non-differentiable, or undefined. If a variable's differentiability\n"
            f"            is not specified, that variable has undefined differentiability.\n"
        )
            ...
            for _, namemap in supportmap:
                for n, schema, versions in namemap:
                    if schema.has_function or schema.has_context_dependent_function:  # type: ignore
                        ...
                        continue
                    s = f'|{support_level_str(schema.support_level)}<a href="#{format_name_with_domain(domain, n)}">{format_name_with_domain(domain, n)}</a>{" (deprecated)" if schema.deprecated else ""}|{format_versions(versions, args.changelog)}|\n'
                    fout.write(s)
            if function_ops:
        ...
        for domain, supportmap in operator_schemas:
            ...
            for _, namemap in supportmap:
                for op_type, schema, versions in namemap:
                    # op_type
                    s = (
                        f'### {support_level_str(schema.support_level)}<a name="{format_name_with_domain(domain, op_type)}"></a><a name="{format_name_with_domain(domain, op_type.lower())}">**{format_name_with_domain(domain, op_type)}**'
                        + (" (deprecated)" if schema.deprecated else "")
                        + "</a>\n"
                    )
