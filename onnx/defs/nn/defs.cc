...
static const char* SkipLayerNormalization_ver24_doc = R"DOC(
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    24,
    OpSchema()
        .SetDoc(SkipLayerNormalization_ver24_doc)
        .Attr(
            "axis",
            "The dimension for layer normalization. If rank(X) is r, axis' allowed range is [-r, r). "
            "Negative value means counting dimensions from the back.",
            AttributeProto::INT,
            static_cast<int64_t>(-1))
        .Attr("epsilon", "The epsilon value to use to avoid division by zero.", AttributeProto::FLOAT, 1e-5f)
        ...
        .SetContextDependentFunctionBodyBuilder(
            [](const FunctionBodyBuildContext& ctx, const OpSchema& schema, FunctionProto& functionProto) {
              ...
              auto* epsilon_attr = ctx.getAttribute("epsilon");
              float epsilon = (epsilon_attr != nullptr) ? epsilon_attr->f() : 1e-5f;
              auto* scaling_factor_attr = ctx.getAttribute("scaling_factor");
...
static const char* SkipRMSNormalization_ver24_doc = R"DOC(
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    24,
    OpSchema()
        .SetDoc(SkipRMSNormalization_ver24_doc)
        .Attr(
            "axis",
            "The dimension for rms normalization. If rank(X) is r, axis' allowed range is [-r, r). "
            "Negative value means counting dimensions from the back.",
            AttributeProto::INT,
            static_cast<int64_t>(-1))
        .Attr("epsilon", "The epsilon value to use to avoid division by zero.", AttributeProto::FLOAT, 1e-5f)
        ...
        .TypeConstraint(
          "T",
          {"tensor(float16)", "tensor(float)", "tensor(double)", "tensor(bfloat16)"},
          "Constrain input X, S (skip) and B (bias) types to float tensors.")
        .TypeConstraint(
          "V",
          {"tensor(float16)", "tensor(float)", "tensor(double)", "tensor(bfloat16)"},
          "Constrain output Y and input gamma type to float tensors.")
        .TypeConstraint("U", {"tensor(float)"}, "Constrain mean and inv_std_var to float tensors.")
        .TypeAndShapeInferenceFunction([](ONNX_NAMESPACE::InferenceContext& ctx) {
          propagateShapeAndTypeFromFirstInput(ctx);
          if (ctx.getNumOutputs() > 1) {
        ...
        .SetContextDependentFunctionBodyBuilder(
            [](const FunctionBodyBuildContext& ctx, const OpSchema& schema, FunctionProto& functionProto) {
              // SkipRMSNormalization  <epsilon> (X, S, gamma, B) => (Y, InputSkipBiasSum)
