...
OPERATOR_SCHEMA(MaxPool).FillUsing(PoolOpSchemaGenerator("MaxPool", "max"));
...
OPERATOR_SCHEMA(LpPool).FillUsing(LpPoolOpSchemaGenerator("LpPool"));
...
OPERATOR_SCHEMA(MaxRoiPool).FillUsing(RoiPoolOpSchemaGenerator("max"));
} // namespace onnx
...
OPERATOR_SCHEMA(Conv).FillUsing(ConvOpSchemaGenerator("a filter"));
