...
3)  Definitions of built-in operators and functions.

Of these, #1 and #2 are covered herein; the built-in operators and functions are covered separately in documents listed at the end of this. Specifically, built-in operators are divided into a set of primitive operators and functions. A function refers to a composed operator which can be semantically represented by a sub-graph (called function body) with using primitive ops. Functionality-wise, an ONNX compatible framework or runtime may inline a function body to execute it if it does not have corresponding implementation of the function.
