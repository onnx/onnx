...
graph_initializer|Graph[]|A list of initializers that are represented by graphs, which indicate the computation that initializes the tensor. Each initializer graph corresponds to a single model graph input with the same name, and has one output with the same dimensions and type as the model graph input.
doc_string|string|A human-readable documentation for this model. Markdown is allowed.
