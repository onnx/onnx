...
5. Write upgrade and downgrade tests:
    1. Add at least one automatic upgrade test for your operator in [tests/python/version_converter/automatic_upgrade_test.py](/tests/python/version_converter/automatic_upgrade_test.py) using `_test_op_upgrade`. These tests create a given operator at a given opset version (usually the version the operator was introduced in) and test that the version converter is able to convert them to the highest available version. So for a new operator `_test_op_upgrade` will not test anything, but as soon as the operator gets updated in a future opset the test will automatically become nontrivial.
    2. Similarly add at least one automatic downgrade test for your operator in [tests/python/version_converter/automatic_downgrade_test.py](/tests/python/version_converter/automatic_downgrade_test.py) using `_test_op_downgrade`. Specifying the current version so that once the op is updated at a higher opset version the test will ensure downward conversion is validated.

6. Update the documentation
    1. Running [the script](/tools/update_doc.sh) to update the doc
...
rank inference at the very least (adding right amount of dimensions to the output shape)
    3. Shape inference functions must be accompanied by unit tests ([tests/python/shape_inference_test.py](/tests/python/shape_inference_test.py)).
...
| Upgrade tests | `tests/python/version_converter/automatic_upgrade_test.py` |
| Downgrade tests | `tests/python/version_converter/automatic_downgrade_test.py` |
